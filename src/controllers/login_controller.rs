...
use crate::{
    dtos::login_dto::LoginDto,
    model_views::administrador_token::AdministradorToken,
    models::erro_json::ErroJson,
    servicos::administrador_servico,
};
...
pub fn login(login_dto_json: Json<LoginDto>) -> Result<status::Custom<Json<AdministradorToken>>, status::Custom<Json<ErroJson>>> {
    ...
    match administrador_servico::login(login_dto.email, login_dto.senha) {
        ...
        Err(str_erro) => Err(status::Custom(Status::BadRequest, Json(ErroJson { 
            erro: str_erro  // ← Aqui usa "erro" em vez de "mensagem"
        }))),
    }
}